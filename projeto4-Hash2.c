...
#define TRUE_EMPTY "$$$$$$$$$$$$$"
#define FAKE_EMPTY "#############"
#define MAX_ITENS_INSERCAO 9
#define MAX_ITENS_BUSCA 5
...
char** carregaRemocao();
char** carregaBusca();
int divisaoInteira(int key); // metodo da divisao inteira
...
void insereRegistro(LIVRO* dados, FILE** fp_biblioteca, FILE** fp_hash) {
    ...
    int soma = 0;
    for (i = 0; i < 13; i++) {
    	soma = soma + (int)(key.isbn[i]);
	}
	printf("soma: %d\n", soma);

    int hash = divisaoInteira(soma);
